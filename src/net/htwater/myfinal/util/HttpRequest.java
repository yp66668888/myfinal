...
public class HttpRequest {
	...
	/**
	 * check the request device is mobile
	 * 
	 * @param request
	 * @return boolean
	 **/
	...
	public static String exec(String url, Map<String, String> params) {
		...
		try {
			...
			out.print(getParams(params).getBytes());
			// flush输出流的缓冲
			out.flush();
	...
	public static String exec(String url) {
		...
		String result = "";
		try {
			URL realUrl = new URL(url.replaceAll(" ", "%20"));
			HttpURLConnection conn = (HttpURLConnection) realUrl.openConnection();
			conn.setConnectTimeout(8000);
			conn.setRequestProperty("Accept", "*/*");
			conn.setRequestProperty("Connection", "Keep-Alive");
